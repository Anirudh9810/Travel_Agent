...
class GroqProvider(OpenAICompatProvider):
    name = "groq"
    capabilities = {**OpenAICompatProvider.capabilities, "reasoning": True, "parallel_tools": False}
    ...
    async def chat(self, messages, *, max_tokens=2048, temperature=0.7, model=None,
        ...
        if tools:
            body["tools"] = self._translate_tools(tools)
            # Disable parallel tool calls — Llama models on Groq generate malformed
            # <function=...{} XML when parallel_tool_calls is enabled.
